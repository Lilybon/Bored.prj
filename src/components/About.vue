<template>
  <v-layout class="about" row wrap>
    <v-flex class="about__info" pa-3 xs12 sm5 md5>
      <h2 class="about__title about__title--main font-weight-light text-xs-center text-sm-left">
        About
        <p class="hidden-sm-and-up headline">● ● ●</p>
      </h2>
      <p
        class="about__paragraph white--text"
      >這是一個找不到該用什麼api好好做side project的junior前端碼農做的小小專案。如果你對做出這鬼東西的人有興趣請點製作者觀看github的其他練習作品，對這API有興趣請看bored api的官方文件。</p>
      <h2 class="about__title white--text font-weight-light">製作者</h2>
      <p class="about__content panel-content--slide-from-right">
        <a href="https://github.com/Lilybon" target="_blank">Lilybon</a>
      </p>
      <h2 class="about__title white--text font-weight-light">API</h2>
      <p class="about__content panel-content--slide-from-right">
        <a href="https://github.com/drewthoennes/Bored-API" target="_blank">Bored-API</a>
      </p>
    ...
    <v-flex class="about__info" pa-3 xs12 sm6 md5>
      <v-form v-model="valid" ref="feedback" lazy-validation>
        ...
        <p
          class="about__paragraph white--text"
        >如果這個無聊專案遇到什麼奇妙的怪bug讓你很不痛快或畫面醜得讓你不忍直視，認為還有值得改進的地方，都歡迎透過以下表格寄信跟我開噴或指教，謝謝！</p>
  ...
  methods: {
    validate() {
      if (this.$refs.feedback.validate()) {
        window.open(
          `mailto:${this.emailAddress}?subject=${this.feedbackTitle}&body=${this.feedbackContent}`
        );
      }
    }
  }

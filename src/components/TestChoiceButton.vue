...
export default {
    ...
    methods: {
        makeChoice(e) {
            if((e.target.nodeName === 'I' && e.target.innerHTML === 'check') 
            || (e.target.nodeName === 'DIV' && e.target.className.indexOf('choice--like') > -1)) {
                this.$emit('makeDecision', true)
                bus.$emit('resetTestCard')
                
            }else if((e.target.nodeName === 'I' && e.target.innerHTML === 'close') 
            || (e.target.nodeName === 'DIV' && e.target.className.indexOf('choice--dislike') > -1)) {
        ...
        dragDrop(e) {
            this.classObject['choice--hover'] = false
            setTimeout(() => {this.makeChoice(e)}, 0)
        }
    }
}
</script>

<template>
    <v-layout class="panel" row wrap justify-space-around pa-2>
            ...
            <v-flex pa-2 xs12 class="panel-content">
                ...
                <p class="panel-content__description panel-content--slide-from-right">
                    {{ likeRateRank.comment }}
                </p>
            </v-flex>
            <StatsChart 
                :data="countGroupByCategory"
                chartType="stats-chart--group-bar-chart"
            />
            <StatsChart 
                :data="actList"
                chartType="stats-chart--scatter-plot"
            />
...
export default {
        ...
        return {
            ...
            likeRateRankList: [
                ...
                },
                {
                    minRate: 0.35,
                    title: '有原則的',
                    comment: '你對活動的好惡有自己獨到的堅持，是個很鮮明的人，但有時候光是聽到不擅長的事情或是考慮到學習成本和難度就會望之卻步，請克服內心的恐懼勇敢的體驗未知的經驗吧。'
                },
                {
                    minRate: 0.1,
                    title: '專一的',
    ...
    computed: {
        likeRate() {
            const likeCount = this.actList.reduce((acc, val) => acc += val.likeFlag ? 1 : 0, 0)
            const totalCount = this.actList.length
            return likeCount / totalCount
        },
        likeRateRank() {
            return this.likeRateRankList.filter(val => this.likeRate >= val.minRate)[0]
        },
